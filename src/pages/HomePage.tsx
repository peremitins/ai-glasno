...
const firstRunSteps = [
  {
    ...
    text: "ИИ разберёт задачи, требования и формат собеседования.",
  },
  {
    title: "Ответьте голосом или текстом",
    text: "Тренировка идёт как короткое интервью с уточнениями.",
  },
  {
    title: "Получите разбор",
    text: "В отчёте будут сильные места, провалы и что улучшить перед встречей.",
  },
];
...
export function HomePage() {
  ...
  return data.activeSessions + data.completedSessions > 0 ? (
    <ReturningDashboard
      activeSession={data.recentSessions.find(
...
function FirstRunDashboard() {
  return (
    <section className="dashboard-mode first-run-dashboard">
      <article className="glass-frame quick-launcher quick-launcher--hero">
        <div className="launcher-copy">
          <p className="page-kicker">Подготовка</p>
          <h1 className="page-title">Бесплатная репетиция собеседования</h1>
          <p className="page-subtitle">
            Вставьте ссылку на вакансию, описание или просто выберите должность.
            Остальное можно уточнить на следующем экране.
          </p>
        </div>
        <QuickStartForm includeResume />
      </article>
      <section className="glass-frame how-it-works">
        <PanelHeading label="Как это работает" title="Три коротких шага" />
      ...
      <section className="glass-frame scenario-panel">
        <PanelHeading
          label="Примеры"
          title="Что можно потренировать"
          action="Настроить подробнее"
        />
        <div className="scenario-grid">
          {quickScenarios.map((scenario) => (
            <Link className="scenario" key={scenario.title} to="/interview/new">
...
function ReturningDashboard({
  ...
  const stats = [
    {
      label: "Сессии",
      value: activeSessions + completedSessions,
      Icon: Rocket,
    },
  ...
  return (
    <section className="dashboard-mode returning-dashboard">
      <article className="glass-frame returning-hero">
        <div>
          <p className="page-kicker">Подготовка</p>
          <h1 className="page-title">Продолжить подготовку</h1>
          <p className="page-subtitle">
            Короткая репетиция по вакансии или профессии.
          </p>
          {activeSession && (
        ...
        <Link
          ...
          to={
            activeSession ? `/interview/${activeSession.id}` : "/interview/new"
          }
        >
          {activeSession ? "Продолжить" : "Начать интервью"}
          <ArrowRight aria-hidden="true" />
        </Link>
      </article>
      <section aria-label="Статистика подготовки" className="stats-strip">
        {stats.map(({ Icon, label, value }) => (
          <article className="glass-frame glass-frame--soft stat" key={label}>
      ...
      <section className="returning-grid">
        <article className="glass-frame quick-launcher">
          <PanelHeading
            label="Быстрый старт"
      ...
      <section className="glass-frame history-panel">
        <PanelHeading
          label="История"
        ...
        {recentSessions.length ? (
          ...
          <p className="muted">
            После первой сессии здесь появится история интервью.
          </p>
        )}
      </section>
    </section>
  ...
  return (
    <form className="launcher-form" noValidate onSubmit={submit}>
      <label className="field" htmlFor={sourceId}>
        <span>Роль или должность</span>
        <span className="input-shell">
          <Search aria-hidden="true" />
          <input
            id={sourceId}
            onChange={(event) => setSource(event.target.value)}
            placeholder="Ссылка, описание вакансии или должность"
            value={source}
          />
        </span>
        <small>Можно выбрать роль из списка или написать свой вариант.</small>
      </label>
      {includeResume && (
        <div className="field">
          <label htmlFor={resumeInputId}>Резюме или опыт</label>
          <textarea
          ...
          <small id={`${resumeInputId}-hint`}>
            PDF, TXT, MD или скан. Учтём информацию на следующем шаге.
          </small>
        </div>
      )}
      <div className="summary-chips">
        <span>Быстро · 3 вопроса</span>
        <span>Смешанное</span>
        <span>Бесплатно</span>
        {includeResume && (
          <Link
            aria-label="Настроить подробнее"
            className="chip-config"
            to="/interview/new"
          >
