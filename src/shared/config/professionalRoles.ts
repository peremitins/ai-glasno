export interface ProfessionalRole {
  id: string;
  name: string;
  // Синонимы для поиска (латиница/англ. написание, сокращения). Отображается
  // всегда name (кириллица), но найти можно и по алиасам: "qa" → "Тестировщик".
  aliases?: string[];
}

export interface ProfessionalRoleCategory {
  id: string;
  name: string;
  roles: ProfessionalRole[];
}

// HH держит одну роль «Программист, разработчик», но для тренажёра интервью
// направление (frontend/backend/mobile/QA/…) определяет сами вопросы. Поэтому
// добавляем недостающие гранулярные IT-подроли внутрь IT-категории HH (id 11).
const IT_EXTRA_ROLES: ProfessionalRole[] = [
  {
    id: "it-frontend",
    name: "Frontend-разработчик",
    aliases: ["frontend", "front-end", "фронтенд", "фронт", "верстальщик"],
  },
  {
    id: "it-backend",
    name: "Backend-разработчик",
    aliases: ["backend", "back-end", "бэкенд", "бэк", "серверный"],
  },
  {
    id: "it-fullstack",
    name: "Fullstack-разработчик",
    aliases: ["fullstack", "full-stack", "фулстек", "фуллстек"],
  },
  {
    id: "it-mobile",
    name: "Мобильный разработчик",
    aliases: ["mobile", "мобильный", "мобайл"],
  },
  { id: "it-ios", name: "iOS-разработчик", aliases: ["ios", "swift", "айос"] },
  {
    id: "it-android",
    name: "Android-разработчик",
    aliases: ["android", "kotlin", "андроид"],
  },
  {
    id: "it-web",
    name: "Web-разработчик",
    aliases: ["web", "веб", "веб-разработчик"],
  },
  {
    id: "it-gamedev",
    name: "Разработчик игр (Game Dev)",
    aliases: ["game developer", "gamedev", "геймдев", "unity", "unreal"],
  },
  {
    id: "it-1c",
    name: "1С-разработчик",
    aliases: ["1c", "1с", "one c", "одинэс"],
  },
  {
    id: "it-embedded",
    name: "Embedded-разработчик",
    aliases: ["embedded", "встраиваемые", "микроконтроллеры", "iot"],
  },
  {
    id: "it-qa-auto",
    name: "QA-автоматизатор (AQA)",
    aliases: [
      "aqa",
      "automation qa",
      "sdet",
      "автотесты",
      "автоматизатор тестирования",
    ],
  },
  {
    id: "it-sre",
    name: "SRE / инженер по надёжности",
    aliases: ["sre", "site reliability", "надёжность"],
  },
  {
    id: "it-ml",
    name: "ML-инженер",
    aliases: ["ml engineer", "machine learning", "мл-инженер", "нейросети"],
  },
  {
    id: "it-data-engineer",
    name: "Data Engineer",
    aliases: ["data engineer", "дата-инженер", "etl", "dwh"],
  },
  {
    id: "it-data-analyst",
    name: "Аналитик данных (Data Analyst)",
    aliases: ["data analyst", "аналитик данных", "sql"],
  },
  {
    id: "it-scrum",
    name: "Scrum-мастер / Agile-коуч",
    aliases: ["scrum", "agile", "скрам", "аджайл"],
  },
  {
    id: "it-designer",
    name: "UX/UI-дизайнер",
    aliases: [
      "ux",
      "ui",
      "ux/ui",
      "дизайнер интерфейсов",
      "продуктовый дизайнер",
      "figma",
    ],
  },
  {
    id: "it-pentester",
    name: "Пентестер (этичный хакер)",
    aliases: ["pentester", "penetration testing", "пентест", "red team"],
  },
  {
    id: "it-architect",
    name: "Архитектор ПО",
    aliases: ["software architect", "архитектор", "solution architect"],
  },
  {
    id: "it-teamlead",
    name: "Тимлид разработки (Team Lead)",
    aliases: ["team lead", "тимлид", "teamlead", "lead"],
  },
  {
    id: "it-blockchain",
    name: "Blockchain-разработчик",
    aliases: ["blockchain", "web3", "solidity", "блокчейн"],
  },
  {
    id: "it-product-owner",
    name: "Product Owner / Владелец продукта",
    aliases: ["product owner", "po", "владелец продукта"],
  },
  {
    id: "it-customer-success",
    name: "Customer Success Manager",
    aliases: ["customer success", "csm", "клиентский успех"],
  },
  {
    id: "it-devrel",
    name: "DevRel-менеджер (Developer Relations)",
    aliases: ["devrel", "developer relations", "developer advocate"],
  },
  {
    id: "it-ai-engineer",
    name: "AI-инженер / Prompt-инженер",
    aliases: [
      "prompt engineer",
      "ai engineer",
      "llm engineer",
      "промпт-инженер",
    ],
  },
  {
    id: "it-ux-researcher",
    name: "UX-исследователь",
    aliases: ["ux researcher", "user research", "юзабилити"],
  },
  {
    id: "it-recruiter",
    name: "IT-рекрутер",
    aliases: ["tech recruiter", "айти-рекрутер", "технический рекрутер"],
  },
];

// Двуязычные алиасы для уже существующих IT-ролей HH (кириллица отображается,
// но найти можно и по латинице): «data scientist» → «Дата-сайентист».
const IT_ALIAS_PATCH: Record<string, string[]> = {
  "Программист, разработчик": [
    "programmer",
    "developer",
    "разработчик",
    "dev",
    "программист",
  ],
  Тестировщик: [
    "qa",
    "tester",
    "quality assurance",
    "тестировщик",
    "ручное тестирование",
  ],
  "Дата-сайентист": ["data scientist", "ds", "ml", "машинное обучение"],
  "DevOps-инженер": ["devops", "девопс", "ci/cd"],
  "Гейм-дизайнер": ["game designer", "геймдизайнер", "game design"],
  "Менеджер продукта": ["product manager", "pm", "продакт", "продукт-менеджер"],
  "Руководитель проектов": ["project manager", "проджект", "пм"],
  "Системный аналитик": ["system analyst", "системный аналитик", "sa"],
  "Бизнес-аналитик": ["business analyst", "бизнес-аналитик", "ba"],
  "Продуктовый аналитик": ["product analyst", "продуктовый аналитик"],
  "BI-аналитик, аналитик данных": ["bi analyst", "power bi", "би-аналитик"],
  "Специалист по информационной безопасности": [
    "security",
    "infosec",
    "cybersecurity",
    "кибербезопасность",
  ],
  "Сетевой инженер": ["network engineer", "сетевик", "cisco"],
  "Системный администратор": [
    "sysadmin",
    "system administrator",
    "админ",
    "linux",
  ],
  "Технический писатель": ["technical writer", "техписатель", "documentation"],
  "Специалист технической поддержки": [
    "support",
    "techsupport",
    "helpdesk",
    "техподдержка",
  ],
  "Руководитель группы разработки": [
    "head of development",
    "dev lead",
    "руководитель разработки",
  ],
  "Дизайнер, художник": ["designer", "дизайнер"],
};

// Локальный снапшот официального справочника HH professional_roles.
// Обновлено вручную 2026-07-08, источник: https://api.hh.ru/professional_roles
// Куратировано вручную 2026-07-08: убраны профессии без культуры структурированного
// собеседования в РФ (ручной труд, рабочие специальности, кастинг/портфолио-найм,
// силовые ведомства) и добавлены недостающие популярные роли.
// IT-категория вынесена в начало списка — основная аудитория тренажёра.
const RAW_PROFESSIONAL_ROLE_CATEGORIES: ProfessionalRoleCategory[] = [
  {
    id: "11",
    name: "Информационные технологии",
    roles: [
      {
        id: "156",
        name: "BI-аналитик, аналитик данных",
      },
      {
        id: "160",
        name: "DevOps-инженер",
      },
      {
        id: "10",
        name: "Аналитик",
      },
      {
        id: "12",
        name: "Арт-директор, креативный директор",
      },
      {
        id: "150",
        name: "Бизнес-аналитик",
      },
      {
        id: "25",
        name: "Гейм-дизайнер",
      },
      {
        id: "165",
        name: "Дата-сайентист",
      },
      {
        id: "34",
        name: "Дизайнер, художник",
      },
      {
        id: "36",
        name: "Директор по информационным технологиям (CIO)",
      },
      {
        id: "73",
        name: "Менеджер продукта",
      },
      {
        id: "155",
        name: "Методолог",
      },
      {
        id: "96",
        name: "Программист, разработчик",
      },
      {
        id: "164",
        name: "Продуктовый аналитик",
      },
      {
        id: "104",
        name: "Руководитель группы разработки",
      },
      {
        id: "157",
        name: "Руководитель отдела аналитики",
      },
      {
        id: "107",
        name: "Руководитель проектов",
      },
      {
        id: "112",
        name: "Сетевой инженер",
      },
      {
        id: "113",
        name: "Системный администратор",
      },
      {
        id: "148",
        name: "Системный аналитик",
      },
      {
        id: "114",
        name: "Системный инженер",
      },
      {
        id: "116",
        name: "Специалист по информационной безопасности",
      },
      {
        id: "121",
        name: "Специалист технической поддержки",
      },
      {
        id: "124",
        name: "Тестировщик",
      },
      {
        id: "125",
        name: "Технический директор (CTO)",
      },
      {
        id: "126",
        name: "Технический писатель",
      },
    ],
  },
  {
    id: "19",
    name: "Автомобильный бизнес",
    roles: [
      {
        id: "62",
        name: "Мастер-приемщик",
      },
      {
        id: "70",
        name: "Менеджер по продажам, менеджер по работе с клиентами",
      },
    ],
  },
  {
    id: "5",
    name: "Административный персонал",
    roles: [
      {
        id: "8",
        name: "Администратор",
      },
      {
        id: "33",
        name: "Делопроизводитель, архивариус",
      },
      {
        id: "76",
        name: "Менеджер/руководитель АХО",
      },
      {
        id: "84",
        name: "Оператор ПК, оператор базы данных",
      },
      {
        id: "88",
        name: "Офис-менеджер",
      },
      {
        id: "93",
        name: "Переводчик",
      },
      {
        id: "110",
        name: "Секретарь, помощник руководителя, ассистент",
      },
    ],
  },
  {
    id: "15",
    name: "Безопасность",
    roles: [
      {
        id: "116",
        name: "Специалист по информационной безопасности",
      },
      {
        id: "120",
        name: "Специалист службы безопасности",
      },
    ],
  },
  {
    id: "26",
    name: "Высший и средний менеджмент",
    roles: [
      {
        id: "26",
        name: "Генеральный директор, исполнительный директор (CEO)",
      },
      {
        id: "36",
        name: "Директор по информационным технологиям (CIO)",
      },
      {
        id: "37",
        name: "Директор по маркетингу и PR (CMO)",
      },
      {
        id: "38",
        name: "Директор по персоналу (HRD)",
      },
      {
        id: "166",
        name: "Директор юридического департамента (CLO)",
      },
      {
        id: "53",
        name: "Коммерческий директор (CCO)",
      },
      {
        id: "80",
        name: "Начальник производства, Главный инженер производства",
      },
      {
        id: "87",
        name: "Операционный директор (COO)",
      },
      {
        id: "157",
        name: "Руководитель отдела аналитики",
      },
      {
        id: "172",
        name: "Руководитель отдела логистики",
      },
      {
        id: "170",
        name: "Руководитель отдела маркетинга и рекламы",
      },
      {
        id: "171",
        name: "Руководитель отдела персонала",
      },
      {
        id: "188",
        name: "Руководитель отдела страхования и кредитования",
      },
      {
        id: "189",
        name: "Руководитель службы эксплуатации",
      },
      {
        id: "161",
        name: "Руководитель филиала",
      },
      {
        id: "125",
        name: "Технический директор (CTO)",
      },
      {
        id: "135",
        name: "Финансовый директор (CFO)",
      },
    ],
  },
  {
    id: "8",
    name: "Добыча сырья",
    roles: [
      {
        id: "27",
        name: "Геодезист",
      },
      {
        id: "28",
        name: "Геолог",
      },
      {
        id: "168",
        name: "Лаборант",
      },
      {
        id: "79",
        name: "Научный специалист, исследователь",
      },
      {
        id: "82",
        name: "Начальник смены, мастер участка",
      },
      {
        id: "49",
        name: "Технолог",
      },
    ],
  },
  {
    id: "14",
    name: "Закупки",
    roles: [
      {
        id: "66",
        name: "Менеджер по закупкам",
      },
      {
        id: "119",
        name: "Специалист по тендерам",
      },
    ],
  },
  {
    id: "24",
    name: "Искусство, развлечения, массмедиа",
    roles: [
      {
        id: "12",
        name: "Арт-директор, креативный директор",
      },
      {
        id: "20",
        name: "Видеооператор, видеомонтажер",
      },
      {
        id: "25",
        name: "Гейм-дизайнер",
      },
      {
        id: "34",
        name: "Дизайнер, художник",
      },
      {
        id: "41",
        name: "Журналист, корреспондент",
      },
      {
        id: "55",
        name: "Копирайтер, редактор, корректор",
      },
      {
        id: "98",
        name: "Продюсер",
      },
    ],
  },
  {
    id: "6",
    name: "Маркетинг, реклама, PR",
    roles: [
      {
        id: "1",
        name: "Event-менеджер",
      },
      {
        id: "2",
        name: "PR-менеджер",
      },
      {
        id: "3",
        name: "SMM-менеджер, контент-менеджер",
      },
      {
        id: "10",
        name: "Аналитик",
      },
      {
        id: "12",
        name: "Арт-директор, креативный директор",
      },
      {
        id: "176",
        name: "Бренд-менеджер",
      },
      {
        id: "34",
        name: "Дизайнер, художник",
      },
      {
        id: "37",
        name: "Директор по маркетингу и PR (CMO)",
      },
      {
        id: "55",
        name: "Копирайтер, редактор, корректор",
      },
      {
        id: "163",
        name: "Маркетолог-аналитик",
      },
      {
        id: "182",
        name: "Менеджер маркетплейсов",
      },
      {
        id: "68",
        name: "Менеджер по маркетингу, интернет-маркетолог",
      },
      {
        id: "70",
        name: "Менеджер по продажам, менеджер по работе с клиентами",
      },
      {
        id: "71",
        name: "Менеджер по работе с партнерами",
      },
      {
        id: "170",
        name: "Руководитель отдела маркетинга и рекламы",
      },
      {
        id: "extra-growth-marketer",
        name: "Growth/Performance-маркетолог",
      },
    ],
  },
  {
    id: "23",
    name: "Медицина, фармацевтика",
    roles: [
      {
        id: "8",
        name: "Администратор",
      },
      {
        id: "15",
        name: "Ассистент врача",
      },
      {
        id: "19",
        name: "Ветеринарный врач",
      },
      {
        id: "24",
        name: "Врач",
      },
      {
        id: "29",
        name: "Главный врач, заведующий отделением",
      },
      {
        id: "42",
        name: "Заведующий аптекой",
      },
      {
        id: "168",
        name: "Лаборант",
      },
      {
        id: "64",
        name: "Медицинская сестра, медицинский брат",
      },
      {
        id: "65",
        name: "Медицинский представитель",
      },
      {
        id: "183",
        name: "Менеджер по качеству",
      },
      {
        id: "79",
        name: "Научный специалист, исследователь",
      },
      {
        id: "151",
        name: "Специалист по сертификации",
      },
      {
        id: "133",
        name: "Фармацевт-провизор",
      },
    ],
  },
  {
    id: "25",
    name: "Наука, образование",
    roles: [
      {
        id: "17",
        name: "Бизнес-тренер",
      },
      {
        id: "23",
        name: "Воспитатель, няня",
      },
      {
        id: "181",
        name: "Куратор/тьютор обучения",
      },
      {
        id: "168",
        name: "Лаборант",
      },
      {
        id: "167",
        name: "Методист",
      },
      {
        id: "79",
        name: "Научный специалист, исследователь",
      },
      {
        id: "101",
        name: "Психолог",
      },
      {
        id: "132",
        name: "Учитель, преподаватель, педагог",
      },
      {
        id: "extra-tutor",
        name: "Репетитор",
      },
    ],
  },
  {
    id: "1",
    name: "Продажи, обслуживание клиентов",
    roles: [
      {
        id: "6",
        name: "Агент по недвижимости",
      },
      {
        id: "10",
        name: "Аналитик",
      },
      {
        id: "154",
        name: "Брокер",
      },
      {
        id: "51",
        name: "Кассир-операционист",
      },
      {
        id: "53",
        name: "Коммерческий директор (CCO)",
      },
      {
        id: "54",
        name: "Координатор отдела продаж",
      },
      {
        id: "57",
        name: "Кредитный специалист",
      },
      {
        id: "70",
        name: "Менеджер по продажам, менеджер по работе с клиентами",
      },
      {
        id: "71",
        name: "Менеджер по работе с партнерами",
      },
      {
        id: "83",
        name: "Оператор call-центра, специалист контактного центра",
      },
      {
        id: "97",
        name: "Продавец-консультант, продавец-кассир",
      },
      {
        id: "105",
        name: "Руководитель отдела клиентского обслуживания",
      },
      {
        id: "106",
        name: "Руководитель отдела продаж",
      },
      {
        id: "188",
        name: "Руководитель отдела страхования и кредитования",
      },
      {
        id: "161",
        name: "Руководитель филиала",
      },
      {
        id: "151",
        name: "Специалист по сертификации",
      },
      {
        id: "121",
        name: "Специалист технической поддержки",
      },
      {
        id: "122",
        name: "Страховой агент",
      },
      {
        id: "129",
        name: "Торговый представитель",
      },
    ],
  },
  {
    id: "7",
    name: "Производство, сервисное обслуживание",
    roles: [
      {
        id: "177",
        name: "Инженер КИПиА",
      },
      {
        id: "174",
        name: "Инженер ПНР",
      },
      {
        id: "44",
        name: "Инженер по качеству",
      },
      {
        id: "45",
        name: "Инженер по охране труда и технике безопасности, инженер-эколог",
      },
      {
        id: "46",
        name: "Инженер по эксплуатации",
      },
      {
        id: "178",
        name: "Инженер слаботочных систем/Инженер связи",
      },
      {
        id: "48",
        name: "Инженер-конструктор, инженер-проектировщик",
      },
      {
        id: "169",
        name: "Инженер-электроник, инженер-электронщик",
      },
      {
        id: "144",
        name: "Инженер-энергетик, инженер-электрик",
      },
      {
        id: "149",
        name: "Контролёр ОТК",
      },
      {
        id: "168",
        name: "Лаборант",
      },
      {
        id: "183",
        name: "Менеджер по качеству",
      },
      {
        id: "152",
        name: "Метролог",
      },
      {
        id: "79",
        name: "Научный специалист, исследователь",
      },
      {
        id: "80",
        name: "Начальник производства, Главный инженер производства",
      },
      {
        id: "82",
        name: "Начальник смены, мастер участка",
      },
      {
        id: "189",
        name: "Руководитель службы эксплуатации",
      },
      {
        id: "111",
        name: "Сервисный инженер, инженер-механик",
      },
      {
        id: "151",
        name: "Специалист по сертификации",
      },
      {
        id: "49",
        name: "Технолог",
      },
    ],
  },
  {
    id: "2",
    name: "Розничная торговля",
    roles: [
      {
        id: "9",
        name: "Администратор магазина, администратор торгового зала",
      },
      {
        id: "35",
        name: "Директор магазина, директор сети магазинов",
      },
      {
        id: "180",
        name: "Категорийный менеджер",
      },
      {
        id: "97",
        name: "Продавец-консультант, продавец-кассир",
      },
      {
        id: "123",
        name: "Супервайзер",
      },
      {
        id: "127",
        name: "Товаровед",
      },
    ],
  },
  {
    id: "9",
    name: "Сельское хозяйство",
    roles: [
      {
        id: "7",
        name: "Агроном",
      },
      {
        id: "19",
        name: "Ветеринарный врач",
      },
      {
        id: "43",
        name: "Зоотехник",
      },
      {
        id: "111",
        name: "Сервисный инженер, инженер-механик",
      },
      {
        id: "49",
        name: "Технолог",
      },
    ],
  },
  {
    id: "21",
    name: "Спортивные клубы, фитнес, салоны красоты",
    roles: [
      {
        id: "8",
        name: "Администратор",
      },
      {
        id: "56",
        name: "Косметолог",
      },
      {
        id: "60",
        name: "Массажист",
      },
      {
        id: "70",
        name: "Менеджер по продажам, менеджер по работе с клиентами",
      },
      {
        id: "138",
        name: "Фитнес-тренер, инструктор тренажерного зала",
      },
    ],
  },
  {
    id: "4",
    name: "Стратегия, инвестиции, консалтинг",
    roles: [
      {
        id: "10",
        name: "Аналитик",
      },
      {
        id: "150",
        name: "Бизнес-аналитик",
      },
      {
        id: "75",
        name: "Менеджер/консультант по стратегии",
      },
      {
        id: "107",
        name: "Руководитель проектов",
      },
      {
        id: "134",
        name: "Финансовый аналитик, инвестиционный аналитик",
      },
    ],
  },
  {
    id: "22",
    name: "Страхование",
    roles: [
      {
        id: "11",
        name: "Андеррайтер",
      },
      {
        id: "91",
        name: "Оценщик",
      },
      {
        id: "122",
        name: "Страховой агент",
      },
    ],
  },
  {
    id: "18",
    name: "Строительство, недвижимость",
    roles: [
      {
        id: "6",
        name: "Агент по недвижимости",
      },
      {
        id: "14",
        name: "Архитектор",
      },
      {
        id: "154",
        name: "Брокер",
      },
      {
        id: "27",
        name: "Геодезист",
      },
      {
        id: "30",
        name: "Главный инженер проекта",
      },
      {
        id: "34",
        name: "Дизайнер, художник",
      },
      {
        id: "47",
        name: "Инженер ПТО, инженер-сметчик",
      },
      {
        id: "45",
        name: "Инженер по охране труда и технике безопасности, инженер-эколог",
      },
      {
        id: "46",
        name: "Инженер по эксплуатации",
      },
      {
        id: "179",
        name: "Инженер строительного контроля",
      },
      {
        id: "48",
        name: "Инженер-конструктор, инженер-проектировщик",
      },
      {
        id: "100",
        name: "Прораб, мастер СМР",
      },
      {
        id: "107",
        name: "Руководитель проектов",
      },
      {
        id: "108",
        name: "Руководитель строительного проекта",
      },
    ],
  },
  {
    id: "10",
    name: "Транспорт, логистика, перевозки",
    roles: [
      {
        id: "159",
        name: "Бортпроводник",
      },
      {
        id: "39",
        name: "Диспетчер",
      },
      {
        id: "67",
        name: "Менеджер по логистике, менеджер по ВЭД, начальник транспортного отдела",
      },
      {
        id: "81",
        name: "Начальник склада",
      },
      {
        id: "172",
        name: "Руководитель отдела логистики",
      },
    ],
  },
  {
    id: "20",
    name: "Туризм, гостиницы, рестораны",
    roles: [
      {
        id: "8",
        name: "Администратор",
      },
      {
        id: "72",
        name: "Менеджер по туризму",
      },
      {
        id: "74",
        name: "Менеджер ресторана",
      },
      {
        id: "76",
        name: "Менеджер/руководитель АХО",
      },
      {
        id: "89",
        name: "Официант, бармен, бариста",
      },
      {
        id: "191",
        name: "Сотрудник ресторана быстрого питания",
      },
    ],
  },
  {
    id: "12",
    name: "Управление персоналом, тренинги",
    roles: [
      {
        id: "17",
        name: "Бизнес-тренер",
      },
      {
        id: "38",
        name: "Директор по персоналу (HRD)",
      },
      {
        id: "181",
        name: "Куратор/тьютор обучения",
      },
      {
        id: "153",
        name: "Менеджер по компенсациям и льготам",
      },
      {
        id: "69",
        name: "Менеджер по персоналу",
      },
      {
        id: "171",
        name: "Руководитель отдела персонала",
      },
      {
        id: "117",
        name: "Специалист по кадрам",
      },
      {
        id: "118",
        name: "Специалист по подбору персонала",
      },
      {
        id: "extra-hrbp",
        name: "HR-бизнес-партнёр (HRBP)",
      },
    ],
  },
  {
    id: "3",
    name: "Финансы, бухгалтерия",
    roles: [
      {
        id: "16",
        name: "Аудитор",
      },
      {
        id: "154",
        name: "Брокер",
      },
      {
        id: "18",
        name: "Бухгалтер",
      },
      {
        id: "50",
        name: "Казначей",
      },
      {
        id: "158",
        name: "Комплаенс-менеджер",
      },
      {
        id: "57",
        name: "Кредитный специалист",
      },
      {
        id: "155",
        name: "Методолог",
      },
      {
        id: "147",
        name: "Специалист по взысканию задолженности",
      },
      {
        id: "134",
        name: "Финансовый аналитик, инвестиционный аналитик",
      },
      {
        id: "135",
        name: "Финансовый директор (CFO)",
      },
      {
        id: "136",
        name: "Финансовый контролер",
      },
      {
        id: "137",
        name: "Финансовый менеджер",
      },
      {
        id: "142",
        name: "Экономист",
      },
    ],
  },
  {
    id: "13",
    name: "Юристы",
    roles: [
      {
        id: "166",
        name: "Директор юридического департамента (CLO)",
      },
      {
        id: "158",
        name: "Комплаенс-менеджер",
      },
      {
        id: "145",
        name: "Юрисконсульт",
      },
      {
        id: "146",
        name: "Юрист",
      },
    ],
  },
  {
    id: "27",
    name: "Другое",
    roles: [
      {
        id: "40",
        name: "Другое",
      },
    ],
  },
];

// Инъекция гранулярных IT-подролей и двуязычных алиасов в IT-категорию HH
// (id 11), сохраняя исходные категории снапшота.
export const PROFESSIONAL_ROLE_CATEGORIES: ProfessionalRoleCategory[] =
  RAW_PROFESSIONAL_ROLE_CATEGORIES.map((category) => {
    const roles = category.roles.map((role) => {
      const patch = IT_ALIAS_PATCH[role.name];
      return patch
        ? { ...role, aliases: [...(role.aliases ?? []), ...patch] }
        : role;
    });
    return category.id === "11"
      ? { ...category, roles: [...IT_EXTRA_ROLES, ...roles] }
      : { ...category, roles };
  });

export const PROFESSIONAL_ROLE_OPTIONS = PROFESSIONAL_ROLE_CATEGORIES.flatMap(
  (category) =>
    category.roles.map((role) => ({
      ...role,
      categoryId: category.id,
      categoryName: category.name,
    })),
);
