...
const requestSchema = z.object({ turnId: z.string().trim().min(1) });
...
export default defineApiRoute(async (event) => {
  ...
  if (!parsed.success) {
    throw apiError("E_VALIDATION", "Не указан текущий вопрос");
  }

  const owner = requireSession(event);
  return new InterviewSessionService(new InterviewRepository()).nextQuestion({
