...
const advancedRequestSchema = z.object({
  ...
  source: z.union([
    ...
    z.object({ type: z.literal("text"), text: z.string().trim().min(10), title: z.string().trim().optional() }),
    z.object({ type: z.literal("profession"), role: z.string().trim().min(2), specialization: z.string().trim().optional() }),
  ]),
  ...
  focus: z.enum(["hr_screening", "professional", "behavioral", "salary_negotiation"]).optional(),
...
function getGoalSettings(goal: AdvancedCreationRequest["sessionGoal"]) {
  if (goal === "quick") return { durationMinutes: 15, questionsCount: 5 };
  if (goal === "deep") return { durationMinutes: 60, questionsCount: 15 };
  return { durationMinutes: 45, questionsCount: 10 };
}

function getVacancy(source: AdvancedCreationRequest["source"]) {
  if (source.type === "text") return source.text;
  if (source.type === "profession") return [source.role, source.specialization].filter(Boolean).join(" · ");
  return source.url;
}
